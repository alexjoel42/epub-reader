...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Wait for window.reader to be available
    function waitForReader(retries = 20) {
        if (window.reader && window.reader.loadEPUB) {
            setupLoadButton();
        } else if (retries > 0) {
            setTimeout(function() { waitForReader(retries - 1); }, 100);
        } else {
            console.error("Reader module not loaded after waiting.");
        }
    }
